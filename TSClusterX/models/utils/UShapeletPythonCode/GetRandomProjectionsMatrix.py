...
def GetRandomProjectionsMatrix(data, sLen, sNum):
    ...
    shValues_flat = []
    for val_list in shValues:
        if isinstance(val_list, list):
            shValues_flat.extend(val_list)
        else:
            shValues_flat.append(val_list)
    
    shStorageValues_flat = []
    for val_list in shStorageValues:
        if isinstance(val_list, list):
            shStorageValues_flat.extend(val_list)
        else:
            shStorageValues_flat.append(val_list)
    
    shValues = np.array(shValues_flat).astype(np.int32)
    shStorageValues = np.array(shStorageValues_flat).astype(np.int32)
    ...
    for i in range(R):
        ...
        for k in range(repIndLen):
            ...
            allTS = shValues[np.argwhere(idxs==1)].flatten()
            allShPositions = shStorageValues[np.argwhere(idxs==1)].flatten()
            tss = allTS
