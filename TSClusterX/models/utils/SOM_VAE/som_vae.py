...
class SOMVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        z_q_up = torch.zeros(x.shape[0], self.latent_dim, device=x.device)
        z_q_up_ = self._gather_nd(self.embeddings, torch.stack([k1_up, k_2], dim=1))
        z_q_up[k1_not_top == 1] = z_q_up_[k1_not_top == 1]
        
        z_q_down = torch.zeros(x.shape[0], self.latent_dim, device=x.device)
        ...
        z_q_right = torch.zeros(x.shape[0], self.latent_dim, device=x.device)
        ...
        z_q_left = torch.zeros(x.shape[0], self.latent_dim, device=x.device)
