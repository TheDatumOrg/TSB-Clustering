...
from .dhwt import transform
from .dist import dist_all
from .dist import shift
...
def _compute_centroids(tseries, assign, num_clusters, to_shift=None):
    ...
    for k in range(num_clusters):
        ...
        if members.any():
            ...
            eig_values, eig_vectors = LA.eigh(m_mat)
            # Find the index of the smallest eigenvalue
            min_idx = np.argmin(eig_values)
            centroids[k] = eig_vectors[:, min_idx]
